      ...
      var viewBoxWidth = 800; // default viewBoxWidth
      var viewBoxHeight = 500; // default viewBoxHeight
      var paddingAmt = 5;
      
      var offset = 20; // used for window size 
      
       var scale = 72; // scale to export correctly for inches
      
      function generateBox(){
        var widthIn = parseFloat($('#width').val());
        var heightIn = parseFloat($('#height').val());
        var depthIn = parseFloat($('#depth').val());
        
        var fullWidthIn = depthIn/2 + widthIn*2 + depthIn*2;
        var fullHeightIn = depthIn+heightIn+depthIn+offset*0.8/scale*2;
...
M${depth/2+width+depth+width} ${depth/2+depth}h${depth} M${depth/2+width+depth+width} ${depth/2+depth+height}h${depth}"/>
    ...
    function addWindow(type){
      ...
      if(type == "rect"){
        var windowPath = document.createElementNS(ns, 'rect');
        ...
        var windowPath = document.createElementNS(ns, 'ellipse');
        windowPath.setAttribute('class', 'window cut');
        windowPath.setAttribute("stroke-width", 1);
        windowPath.setAttribute("fill", "none");
      ...
      var newSVG = (new XMLSerializer).serializeToString($('.box svg')[0]);
    ...
    $('.generate').on('click', function(){
      ...
      var activeWindowType = $('.windowSelector button.active').attr('id')
    ...
    function parseSTL(){
      ...
      var fileName = $('#stlFile')[0].files[0].name;
      ...
      if(fileExt == "stl"){
         ...
         formData.append('file', $('#stlFile')[0].files[0]);
